...
namespace RemoteCW
{
    public class AudioDriver
    {
        ...
        public AudioDriver(KeyDriver key)
        {
            this.key = key;
            GenerateSineTone();
            ProcessStartInfo psi = new ProcessStartInfo("paplay", "-p --client-name RemoteCW --stream-name Key --latency 1000 --rate 48000 --channels 1 --format s16le --volume 16384 --raw");
